...
- **하이브리드 실시간 스크래핑 & 안전 랜딩**: 구글 뉴스 RSS를 통해 실시간으로 현재 진행 중인 이벤트의 **제목과 상세 요약**을 동적으로 파싱하되, 랜딩 시에는 엉뚱한 뉴스 기사가 아닌 수동 매핑된 30여 개 **브랜드 공식 이벤트 페이지**로 100% 강제 고정(Fallback)하여 사용자 경험을 극대화했습니다.
- **거지맵(가성비 식당) 스마트 딥링크**: 거지맵 마커 클릭 시 단순 메인 페이지 이동에 그치지 않고, 해당 식당의 **상세 주소를 거지맵 검색 파라미터(`?q=`)로 자동 주입**하여 해당 식당 정보 화면으로 즉시 랜딩되도록 연동성을 강화했습니다.
...
└── tests/                     # pytest 기반 62개 자동화 테스트 스위트
    ...
    ├── test_usecases.py
    ├── test_r1_dynamic_scraper.py
    ├── test_r2_popup_details.py
    └── test_r3_regression_prevention.py
```
